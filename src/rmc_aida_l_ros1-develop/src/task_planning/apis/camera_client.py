...
class CameraClient:
    def __init__(self):
        ...
        # 加载yolo模型
        self.model = YOLO(os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../yolo/best2.pt"))
        # 加载SAM模型
        ...
        # 相机内参
        self.color_intr = {"ppx": 292.794675, "ppy": 275.145308, "fx": 660.609370, "fy": 656.300601}

        # 旋转矩阵和位移向量
        self.rotation_matrix = np.array([[9.42887983e-01, -3.15123418e-01, 1.07979084e-01],
                                    [3.29406364e-01, 9.30261836e-01, -1.61568447e-01],
                                    [-4.95348198e-02, 1.87909944e-01, 9.80936366e-01]])
        self.translation_vector = np.array([9.30302738e+01, -7.15735229e+01, 4.91918941e+02])
    ...
    def detect_all_objects(self, image, conf_threshold=0.5):
        """
    ...
    def get_camera_coords(self, mask, color_img, depth_img=None):
        if mask is not None:
            ...
            if obj_pixel is not None:
                ...
                # 像素坐标转相机坐标
                x = (cx - self.color_intr["ppx"]) * depth / self.color_intr["fx"]
                y = (cy - self.color_intr["ppy"]) * depth / self.color_intr["fy"]
                z = depth
                # 获取相机系坐标
                obj_cam = np.array([x, y, z])
